$(function () {
  ...
  $(".star-rating").each(function () {
    ...
    $.post(processURL, { action: "get_rating", id: ratingId }).done(function (
      data
    ) {
      if (data["result"] === "success") {
        var ratingAvg = parseFloat(data["data"]["rating_avg"]),
          totalVotes = data["data"]["total_votes"];
        $(_this)
          .find(".star-rating__live")
          .css("width", (ratingAvg.toFixed(1) / maxStars) * 100 + "%");
        $(_this)
          .closest(".star-rating__wrapper")
          .find(".star-rating__avg")
          .text(ratingAvg.toFixed(1));
        $(_this)
          .closest(".star-rating__wrapper")
          .find(".star-rating__votes_count")
          .text(
            totalVotes + numToStr(totalVotes, [" оценка", " оценки", " оценок"])
          );
  ...
  starRatingItems.on("mouseover", function () {
    ...
    $(this)
      ...
      .addClass("d-none")
      .end()
      .find(".star-rating__votes_message")
      .removeClass("d-none");
  ...
  starRatingItems.on("mouseout", function () {
    ...
    $(this)
      ...
      .addClass("d-none");
    $(this)
      .closest(".star-rating__live")
      .find(".star-rating__item")
      .removeClass("star-rating__item_active");
  });
    ...
    $.post(processURL, {
      ...
      if (!$.isEmptyObject(data)) {
        if (data["result"] === "success") {
          ...
          $(_this)
            .closest(".star-rating")
            .removeClass("star-rating_active")
            .find(".star-rating__item_active")
            .removeClass("star-rating__item_active")
            .end()
            .find(".star-rating__live")
            .css("width", (ratingAvg.toFixed(1) / maxStars) * 100 + "%");
          $(_this)
            ...
            .text(
              totalVotes +
                numToStr(totalVotes, [" оценка", " оценки", " оценок"])
            );
          if (localStorage.getItem("star_rating")) {
            output = JSON.parse(localStorage.getItem("star_rating"));
          }
